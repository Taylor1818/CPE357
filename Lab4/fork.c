...
void main(int args, char *arg[]){
    ...
    if (((float)atof(arg[3]) != 0) && ((float)atof(arg[3]) != 1)){
        printf("Invalid Parallel Selection\n0: No Parellel\n1: Parallel\n");
        return;
    }
        ...
        if(0==fork()){
            for(int y=halfHeight; y<bmp1.HEIGHT; y++){
                for(int x =0; x<bmp1.WIDTH; x++){
                    setBlue(x, y, bmpOut, min(255, ((ratio*255.0)+getBlue(x,y,bmp1))));
                    setRed(x, y, bmpOut, min(255, ((ratio*255.0)+getRed(x,y,bmp1))));
                    setGreen(x, y, bmpOut, min(255, ((ratio*255.0)+getGreen(x,y,bmp1))));
                }
            }
            return;
        } else {
